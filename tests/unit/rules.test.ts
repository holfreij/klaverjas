	...
	describe('getLegalMoves', () => {
		...
		describe('when hand has led suit', () => {
			it('should only allow cards of the led suit', () => {
				...
				const trick: PlayedCard[] = [
					{ card: { suit: 'spades', rank: '10' }, player: 0 },
				];

				const legal = getLegalMoves(hand, trick, trump);

				expect(legal).toHaveLength(2);
				expect(legal).toContainEqual({ suit: 'spades', rank: 'A' });
				expect(legal).toContainEqual({ suit: 'spades', rank: '7' });
			});
		});
		...
		describe('when hand has no led suit but has trump', () => {
			it('should require playing trump when trick has no trump', () => {
				const hand: Card[] = [
					{ suit: 'hearts', rank: 'J' }, // trump
					{ suit: 'hearts', rank: '7' }, // trump
					{ suit: 'clubs', rank: 'K' },
				];
				const trick: PlayedCard[] = [
					{ card: { suit: 'spades', rank: 'A' }, player: 0 },
				];
			...
			it('should require higher trump when trick has trump and player can beat it', () => {
				const hand: Card[] = [
					{ suit: 'hearts', rank: 'J' }, // trump Nel - can beat
					{ suit: 'hearts', rank: '7' }, // trump - cannot beat
					{ suit: 'clubs', rank: 'K' },
				];
				const trick: PlayedCard[] = [
					{ card: { suit: 'spades', rank: 'A' }, player: 0 },
					{ card: { suit: 'hearts', rank: '9' }, player: 1 }, // trump Nell
				];

				const legal = getLegalMoves(hand, trick, trump);

				expect(legal).toHaveLength(1);
				expect(legal[0]).toEqual({ suit: 'hearts', rank: 'J' });
			});

			it('should allow any trump when trick has trump and player cannot beat it (under-trump)', () => {
				const hand: Card[] = [
					{ suit: 'hearts', rank: 'A' }, // trump - cannot beat J
					{ suit: 'hearts', rank: '7' }, // trump - cannot beat J
					{ suit: 'clubs', rank: 'K' },
				];
				const trick: PlayedCard[] = [
					{ card: { suit: 'spades', rank: 'A' }, player: 0 },
					{ card: { suit: 'hearts', rank: 'J' }, player: 1 }, // trump Nel
				];
			...
			it('should enforce Rotterdam rule: must trump even if partner is winning', () => {
				...
				const trick: PlayedCard[] = [
					{ card: { suit: 'spades', rank: '7' }, player: 0 },
					{ card: { suit: 'spades', rank: 'A' }, player: 1 },
				];
				...
				expect(legal).toHaveLength(1);
				expect(legal[0]).toEqual({ suit: 'hearts', rank: '7' });
			});
		});
		...
		describe('edge cases', () => {
			...
			it('should handle trump being led', () => {
				const hand: Card[] = [
					{ suit: 'hearts', rank: 'A' }, // trump
					{ suit: 'hearts', rank: '7' }, // trump
					{ suit: 'spades', rank: 'K' },
				];
				const trick: PlayedCard[] = [
					{ card: { suit: 'hearts', rank: 'J' }, player: 0 }, // trump led
				];
			...
			it('should handle only having one legal card', () => {
				...
				expect(legal).toHaveLength(1);
				expect(legal[0]).toEqual({ suit: 'spades', rank: '7' });
			});
		});
	});
	...
	describe('getTrickWinner', () => {
		...
		it('should return highest card of led suit when no trump played', () => {
			const trick: PlayedCard[] = [
				{ card: { suit: 'spades', rank: '7' }, player: 0 },
				{ card: { suit: 'spades', rank: 'A' }, player: 1 },
				{ card: { suit: 'spades', rank: '10' }, player: 2 },
		...
		it('should return highest trump when trump was played', () => {
			const trick: PlayedCard[] = [
				{ card: { suit: 'spades', rank: 'A' }, player: 0 },
				{ card: { suit: 'hearts', rank: '7' }, player: 1 }, // trump
				{ card: { suit: 'hearts', rank: '9' }, player: 2 }, // trump Nell
				{ card: { suit: 'spades', rank: 'K' }, player: 3 },
			];
		...
		it('should return trump Jack (Nel) as highest when played', () => {
			const trick: PlayedCard[] = [
				...
				{ card: { suit: 'hearts', rank: 'J' }, player: 1 }, // trump Nel
				{ card: { suit: 'hearts', rank: '9' }, player: 2 }, // trump Nell
				{ card: { suit: 'hearts', rank: 'A' }, player: 3 }, // trump Ace
			];
		...
		it('should ignore cards that don\'t follow suit and aren\'t trump', () => {
			const trick: PlayedCard[] = [
				{ card: { suit: 'spades', rank: '7' }, player: 0 },
				{ card: { suit: 'clubs', rank: 'A' }, player: 1 }, // doesn't follow, not trump
				{ card: { suit: 'spades', rank: '8' }, player: 2 },
				{ card: { suit: 'diamonds', rank: 'A' }, player: 3 }, // doesn't follow, not trump
			];

			expect(getTrickWinner(trick, trump)).toBe(2); // 8 of spades is highest spade
		});

		it('should handle led suit being trump', () => {
			const trick: PlayedCard[] = [
				{ card: { suit: 'hearts', rank: '7' }, player: 0 }, // trump led
				{ card: { suit: 'hearts', rank: 'A' }, player: 1 }, // trump
				{ card: { suit: 'hearts', rank: '9' }, player: 2 }, // trump Nell
				{ card: { suit: 'hearts', rank: '8' }, player: 3 }, // trump
			];
		...
		it('should return first player for single-card trick', () => {
			const trick: PlayedCard[] = [
				{ card: { suit: 'spades', rank: '7' }, player: 2 },
			];
